#!/usr/bin/python3
"""
Creates a new view on State object
"""
...
def get_all_states():
    """
    Retrives all the list of state objects
    """
    # Get all State objects from the storage
    states = storage.all(State).values()
    # Converta all objects to dictionaries and jsonify the list
    state_list = [state.to_dict() for state in states]
    return jsonify(state_list)

# Route for retrieving a specified State obj by ID
@app_views.route('/states/<state_id>', methods=['GET'], strict_slashes=False)
def get_state(state_id):
    """
    Retrieves a state obj
    """
    # Gets a State object with a given ID from storage
